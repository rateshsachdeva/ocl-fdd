...
def _findings_slide(prs: Presentation, analysis: AnalysisResult) -> None:
    ...
    if not findings:
        frame.paragraphs[0].text = "No material deterministic findings were triggered by the available data."
        return
...
def _table_slide(prs: Presentation, title: str, headers, rows) -> None:
    ...
    table = slide.shapes.add_table(max_rows + 1, max_cols, Inches(0.6), Inches(1.5), Inches(12.1), Inches(5.5)).table
    ...
    for row_index, row in enumerate(rows[:max_rows], start=1):
        for column in range(max_cols):
            table.cell(row_index, column).text = _format_value(row[column] if column < len(row) else "")
