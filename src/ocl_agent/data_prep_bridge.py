...
def run_full_data_preparation(repo_root: Path, source_dir: Path, work_root: Path) -> DataPrepBridgeResult:
    ...
    Within that source-specific workspace, a previously published
    ``output/latest`` is reused before a new upstream execution is created.
    """
    ...
    if cached_manifest:
        ...
        status = {
            "state": "PUBLISHED_CACHE_REUSED",
            "execution_id": cached_execution_id,
            "final_execution_status": cached_status,
            "next_actor": "NONE",
        ...
        warnings = _published_package_warnings(
            bootstrap,
            source_dir=source_dir,
            source_fingerprint=source_fingerprint,
            published_status=cached_status,
        )
        return DataPrepBridgeResult(
            state="DATA_PREP_READY",
            upstream_state="PUBLISHED_CACHE_REUSED",
            work_root=runs_root,
            output_root=output_root,
            standardized_output=latest,
            source_fingerprint=source_fingerprint,
