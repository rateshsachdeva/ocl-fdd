...
from ocl_agent.part1_databook.semantic_handoff import DatasetUsage, SemanticHandoff, load_semantic_handoff, write_semantic_handoff_draft
...
def run_part1(standardized_output: Path, config_dir: Path, output_dir: Path) -> Part1Result:
    ...
    controls = build_core_controls(build.records, build, handoff, judgment_issues, package)
    ...
    if build.issues or judgment_issues:
        return Part1Result("AWAITING_JUDGMENT_REVIEW", package, judgments, input_review, handoff=handoff, build=build, semantic_review=semantic_review, review_context=review_context, judgment_issues=judgment_issues, controls=controls)
    ...
    if blocking_controls:
        return Part1Result("AWAITING_CONTROL_ALIGNMENT", package, judgments, input_review, handoff=handoff, build=build, semantic_review=semantic_review, review_context=review_context, controls=controls)
    usages = {usage for binding in handoff.datasets for usage in binding.usages}
    blueprint = build_blueprint(build.records, source_dataset_files=[path.name for path in package.datasets], has_rollforward_data=DatasetUsage.MOVEMENT_RECORDS in usages, supported_analyses=())
    ...
    reopened.close()
    return Part1Result("DATABOOK_READY", package, judgments, input_review, handoff=handoff, build=build, semantic_review=semantic_review, review_context=review_context, controls=controls, blueprint=blueprint, databook=databook)
