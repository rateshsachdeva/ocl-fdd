...
from ocl_agent.part2_analysis.engine import ABSOLUTE_MATERIALITY
    ...
    for label, category in sorted(labels):
        ...
        if old == 0 and abs(new) >= ABSOLUTE_MATERIALITY:
            results.append(Finding(
                ...
                "NEW_ITEM",
                {"source_label": label, "category": category, "previous_period": previous, "latest_period": latest, "latest": str(new), "materiality": "MATERIAL"},
                "MEDIUM",
            ))
            ...
            results.append(Finding(
                ...
                (label, previous, latest),
                "CLIFF",
                {"source_label": label, "category": category, "previous_period": previous, "latest_period": latest, "previous": str(old), "materiality": "MATERIAL"},
                "MEDIUM",
            ))
    return results
...
def _stale(records: list[OCLRecord], *, minimum_months: int = 4) -> list[Finding]:
    ...
    for label, category in sorted(labels):
        ...
        if abs(series[0]) >= ABSOLUTE_MATERIALITY and all(value == series[0] for value in series[1:]):
            results.append(Finding(
                ...
                (label, tail[0], tail[-1]),
                "STALE_BALANCE",
                {"source_label": label, "category": category, "amount": str(series[0]), "start_period": tail[0], "end_period": tail[-1], "months": minimum_months, "materiality": "MATERIAL"},
                "MEDIUM",
            ))
    return results
