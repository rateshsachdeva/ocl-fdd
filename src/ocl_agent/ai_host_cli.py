...
- Read only the instruction, handoff, review context, prepared profile, reusable-knowledge context and other evidence explicitly referenced by the coordination payload or explicitly linked from those files.
...
- When the action is UNDERSTAND_AND_PLAN and `reusable_knowledge_context` is present in the coordination payload, read that compact context FIRST. Use its matched patterns to shortcut familiar structural/accounting interpretation. Do not re-derive already-familiar patterns unless the current profile contradicts them. Current profile/sample/source evidence always overrides reusable knowledge.
- The reusable-knowledge context is NOT an answer key and must never be used to force a field meaning, category, scope or conclusion. Do NOT open synthetic golden-truth workbooks, Expected_Results files, generation-validation files or historical training prompts as part of a production planning checkpoint.
- For UNDERSTAND_AND_PLAN, spend reasoning effort on genuine ambiguity: changed grain, conflicting field meanings, multiple logical datasets, unclear period/amount semantics, incomplete relationships and evidence limitations. Preserve source-present supporting FDD datasets when evidence shows they may support OCL analysis, including monthly P&L/expense data, detailed accrued-liability schedules, movement/reversal/settlement or subsequent-payment data, payroll/revenue context, contracts/projects and account mapping. Preserve useful evidence fields such as dates, vendor/counterparty, document or obligation identifiers, descriptions, movement type, settlement/payment date, expected amount and related expense category when they exist. Do not discard supporting data merely because it is not the core annual OCL listing; describe its role in the Dataset Map / Processing Plan from the supplied evidence.
