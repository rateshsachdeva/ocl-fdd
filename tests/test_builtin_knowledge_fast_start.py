...
def test_builtin_knowledge_pack_exists_and_contains_benchmark_patterns():
    ...
    assert "Cat" in text
    assert "Map1" in text
    assert "Actual / Budget / Variance" in text
    assert "multiple independent logical datasets" in text
    assert "post-close settlement" in text
    assert "adequacy" in text
...
def test_understand_and_plan_coordination_gets_fast_start_knowledge(tmp_path: Path):
    ...
    data_prep_bridge._attach_builtin_planning_knowledge(coordination, repo_root)
