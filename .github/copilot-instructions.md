...
When an interactive coding-agent session is itself running the skill, continue automatically through every coordination checkpoint where `next_actor=AI_HOST` and `must_continue=true`, as described in `AGENTS.md`, until `READY`, `HUMAN`, or `FAILED`.
